...
SELECT 
    anio,
    mes,
    total_cantidad AS total_ventas,
    AVG(total_cantidad) OVER () AS promedio_mensual
...
WITH Gastos_Clientes AS (
    SELECT 
        c.cliente_id,
        UPPER(c.nombre) AS nombre_completo,  -- Solo usamos el campo `nombre` aquí, si `apellido` no existe
        SUM(CAST(p.precio AS NUMBER(10, 2)) * v.cantidad) AS total_gasto,  -- Total de gasto como decimal
        AVG(p.precio * v.cantidad) AS gasto_promedio  -- Promedio de gasto por cliente
    FROM Ventas v
    JOIN Clientes c ON v.cliente_id = c.cliente_id
    JOIN Productos p ON v.producto_id = p.producto_id
    WHERE v.fecha >= ADD_MONTHS(SYSDATE, -12)  -- Ventas del último año
    AND EXTRACT(YEAR FROM c.fecha_registro) = 2023  -- Clientes registrados en el año 2023
    GROUP BY c.cliente_id, c.nombre
)
SELECT cliente_id, nombre_completo, total_gasto, gasto_promedio
...
SELECT 
    ...
    departamento,
    fecha_contratacion,
    TRUNC(MONTHS_BETWEEN(SYSDATE, fecha_contratacion) / 12) AS antiguedad_en_anos
FROM Empleados
